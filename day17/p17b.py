...
#(row,col),DIR,blocks_moved
start = ((0,0),Dir.RIGHT,0)
...
def neigh(state):
    neighbors = []
    #ic(state)
    (row,col),d,blocks_moved = state
    if blocks_moved < 4 and blocks_moved > 0: # 0 is for start
        newr,newc = move_dir(row,col,d)
        if newr < 0 or newc < 0 or newr >= ROWS or newc >= COLS:
            return []
        else:
            return [((newr,newc),d,blocks_moved+1)]
    ...
    for possd in DIRECTIONS:
        ...
        if newr < 0 or newc < 0 or newr >= ROWS or newc >= COLS:
            ...
            if blocks_moved >= 10:
                ...
                neighbors.append(((newr,newc),possd,blocks_moved+1))
        else: # new direction
            neighbors.append(((newr,newc),possd,1)) # moved one block in that direciton so far
    #ic(state, neighbors)
...
class MyAStar(astar.AStar):
    ...
    def is_goal_reached(self, current, goal):
        return current[0] == goal[0] and current[2] >= 4 # only compare coords

myas = MyAStar(themap)
res = list(myas.astar(start,goal))
ic(res)
ic(len(res))
def draw_map_with_path(mapin,path):
    for rownum,row in enumerate(mapin):
        for colnum,val in enumerate(row):
            if any(row==rownum and col==colnum for (row,col),d,s in path):
                p(f"[red]{val}[/red]",end="")
            else:
                p(f"{val}",end="")
        print()

draw_map_with_path(themap,res)
